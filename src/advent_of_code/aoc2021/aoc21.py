...
def calc_rolls_and_losing_score(start_positions: tuple[int, ...]) -> tuple[int, int]:
    turns_to_win = tuple(
        count_turns_to_score(player, start, 1000) for player, start in enumerate(start_positions)
    )
...
@pytest.mark.parametrize(("turn_count", "winning_player", "expected"), ((166, 1, 993), (1, 0, 6)))
