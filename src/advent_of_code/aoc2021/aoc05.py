...
class AocSolution(Solution[int, int]):
    def __init__(self, **kwargs):
        super().__init__(5, 2021, **kwargs)

    def solve_part_one(self) -> int:
        with self.open_input() as f:
            segments = translate_to_segments(f.readlines())
            intersection_counter = count_intersections(segments, orthogonal_only=True)
        return sum(1 for p in intersection_counter if intersection_counter[p] > 1)

    def solve_part_two(self) -> int:
        with self.open_input() as f:
            segments = translate_to_segments(f.readlines())
            intersection_counter = count_intersections(segments, orthogonal_only=False)
        return sum(1 for p in intersection_counter if intersection_counter[p] > 1)
