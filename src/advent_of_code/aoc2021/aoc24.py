...
class AocSolution(Solution[int, int]):
    def __init__(self, **kwargs):
        super().__init__(24, 2021, **kwargs)

    def solve_part_one(self) -> int:
        with self.open_input() as f:
            monad = list(f)
        return find_largest_valid_number(monad)

    def solve_part_two(self) -> int:
        with self.open_input() as f:
            monad = list(f)
        return find_smallest_valid_number(monad)
...
def search_implementation(validator: Program, target: SearchTarget) -> Optional[int]:
    ...
    while len(states) > 0:
        ...
        for n in target.digit_range:
            ...
            if new_state not in values or target.comparator(new_value, values[new_state]):
...
@pytest.mark.parametrize(
    ("value", "expected"), ((15, (1, 1, 1, 1)), (1, (0, 0, 0, 1)), (10, (1, 0, 1, 0)))
)
