...
class AocSolution(Solution[int]):
    def __init__(self, **kwargs):
        super().__init__(23, 2021, **kwargs)

    def solve_part_one(self) -> int:
        with self.open_input() as f:
            initial_state = read_state(f)
        return find_minimal_cost_to_solution(initial_state)

    def solve_part_two(self) -> int:
        with self.open_input() as f:
            initial_state = read_state(extend_state(f))
        return find_minimal_cost_to_solution(initial_state, build_board(extended=True))
...
class AmphipodState:
    ...
    def at_destination(self):
        return not self.position.in_hallway() and self.position.x == self.type.destination_x
...
def read_state(fp: TextIO) -> BoardState:
    ...
    for y, line in enumerate(fp):
        for x, c in enumerate(line[1:]):
            if c in ("A", "B", "C", "D"):
                amphipods.append(AmphipodState(type=AmphipodType(c), position=Position(x, y)))
...
@pytest.mark.parametrize(("name", "movement_cost"), (("A", 1), ("B", 10), ("C", 100), ("D", 1000)))
