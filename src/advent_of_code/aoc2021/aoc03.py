...
class AocSolution(Solution[int]):
    def __init__(self):
        super().__init__(3, 2021)

    def solve_part_one(self) -> int:
        with self.open_input() as f:
            diagnostic_readings = read_diagnostics(f)
        return calc_power_consumption(diagnostic_readings)

    def solve_part_two(self) -> int:
        with self.open_input() as f:
            diagnostic_readings = read_diagnostics(f)
        return calc_life_support_rating(diagnostic_readings)
