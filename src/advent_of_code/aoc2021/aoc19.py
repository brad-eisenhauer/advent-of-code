...
class AocSolution(Solution[int, int]):
    ...
    @property
    def composite_report(self) -> CompositeReport:
        if self._composite_report is None:
            with self.open_input() as f:
                scanner_reports = list(parse_input(f))
            self._composite_report = CompositeReport.build_composite_report(scanner_reports)
        return self._composite_report

    def solve_part_one(self) -> int:
        return len(self.composite_report.get_all_beacons())

    def solve_part_two(self) -> int:
        return max(
            calc_manhattan_distance(left, right)
            for left, right in combinations(self.composite_report.scanners, 2)
        )
    ...
    def generate_rotations(self) -> Iterator[ScannerReport]:
        ...
        for _ in range(2):
            for _ in range(3):
                ...
                # X->Y, Y->Z, Z->X
                next_variation = next_variation.rotate(Axis.Z, Direction.NEGATIVE).rotate(
...
class CompositeReport:
    ...
    @classmethod
    def build_composite_report(cls, scanner_reports: Sequence[ScannerReport]) -> CompositeReport:
