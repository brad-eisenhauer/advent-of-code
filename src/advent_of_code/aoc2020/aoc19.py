...
class AocSolution(Solution[int, int]):
    ...
    def read_input(self) -> tuple[RuleBuilder, list[str]]:
        with self.open_input() as f:
            rules = takewhile(lambda line: line != "", (line.strip() for line in f))
            builder = RuleBuilder(rules)
            messages = [line.strip() for line in f]
        return builder, messages

    def solve_part_one(self) -> int:
        builder, messages = self.read_input()
        rule_0 = builder.get_rule(0)
        return sum(1 for message in messages if rule_0.is_match(message))
